...
export default function CameraPlayer({
      ...
      try {
        ...
        if (video.canPlayType("application/vnd.apple.mpegurl")) {
          ...
          hls.on(Hls.Events.ERROR, (_event, errData) => {
            ...
            console.warn(
              `[Camera ${cameraId}] HLS fatal xato:`,
              errData.type,
              errData.details,
            );
            ...
            switch (errData.type) {
              case Hls.ErrorTypes.NETWORK_ERROR:
                ...
                if (retryCount < maxRetries) {
                  retryCount++;
                  console.log(
                    `[Camera ${cameraId}] Network qayta ulanish ${retryCount}/${maxRetries}...`,
                  );
        ...
        if (retryCount < maxRetries) {
          retryCount++;
          console.log(
            `[Camera ${cameraId}] Stream POST xatosi, ${retryCount}/${maxRetries} qayta urinish...`,
          );
          retryTimeoutRef.current = setTimeout(() => {
            if (!cancelled) startStream();
          }, 2000);
  ...
  // locked bo'lganda videoni majburan mute qilish (native fullscreen controls ham bloklanadi)
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;
    ...
    if (locked) {
      ...
      // Fullscreen native controls orqali unmute qilishni bloklash
      const forceMute = () => {
        if (!video.muted) {
          video.muted = true;
          setIsMuted(true);
    ...
    <div className="relative bg-black rounded-lg overflow-hidden border border-neutral-800 group">
      ...
      <div className="absolute top-0 left-0 right-0 z-10 bg-gradient-to-b from-black/80 to-transparent p-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div
            className={`w-2 h-2 rounded-full ${online && status === "playing" ? "bg-green-500 animate-pulse" : "bg-red-500"}`}
          />
      ...
      {status === "playing" && (
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3 flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={locked ? undefined : toggleMute}
          ...
          {onSnapshot && (
            <button
              onClick={locked ? undefined : onSnapshot}
              className={`p-2 rounded-md transition-colors ${
                locked
                  ? "bg-white/5 cursor-not-allowed opacity-30 text-white/30"
                  : "bg-white/10 hover:bg-white/20 text-white"
              }`}
              title={locked ? "Qulflangan" : "Snapshot olish"}
              disabled={locked}
            >
